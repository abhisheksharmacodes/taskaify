...
export async function PUT(req: NextRequest, { params }: { params: { id: string, subtaskId: string } }) {
  ...
  const subtaskRows = await db.select().from(subtasks).where(and(eq(subtasks.id, Number(params.subtaskId)), eq(subtasks.taskId, Number(params.id))));
  ...
  const updated = await db.update(subtasks)
    .set(updateData)
    .where(and(eq(subtasks.id, Number(params.subtaskId)), eq(subtasks.taskId, Number(params.id))))
    .returning();
  if (updated.length === 0) {
    return NextResponse.json({ error: 'Subtask not found or not yours' }, { status: 404 });
  }
